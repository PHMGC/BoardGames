...
// Implementação da classe TTTPiece
TTTPiece::TTTPiece(const int turn, const std::string &player)
    : Piece(turn, player) {}

bool TTTPiece::isValidMove(std::array<size_t, 2> startPos, std::array<size_t, 2> endPos) const {
    return true;
}

// Implementação da classe TicTacToe
TicTacToe::TicTacToe() : Game({3, 3}, "Jogo da Velha") {}

bool TicTacToe::playTurn() {
...
bool TicTacToe::checkLine(const std::array<size_t, 2> pos1,
